...
async function ensureBucket() {
  ...
  const r = getRecursiv();
  await r.storage.ensureBucket({ name: BUCKET, project_id: PROJECT_ID });
  _ensured = true;
}
...
export async function getPresignedUploadUrl(input: { key: string; contentType: string }) {
  await ensureBucket();
  const r = getRecursiv();
  const { data } = await r.storage.getUploadUrl({
    bucket_name: BUCKET,
...
export async function listFiles() {
  await ensureBucket();
  const r = getRecursiv();
  const { data } = await r.storage.listItems({
    bucket_name: BUCKET,
    project_id: PROJECT_ID!,
  });
